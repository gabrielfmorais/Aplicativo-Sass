...
- **Regras centrais:**
  ...
  - Execução sem agendamento (ad hoc) é permitida (`scheduled_care_id NULL`, `care_type` obrigatório) — ✅ **implementada na SPEC-052** pela RPC `record_ad_hoc_care`; o tipo do domínio torna *"avulsa sem `care_type_code`"* impossível de escrever (união discriminada). ⚠️ **Ela não é cronograma:** fica fora de `overdue`/`today`/`upcoming` (`buildAdHocHistory`, não `buildTodayView`), não conta aderência, e não paga ponto — `journey_points` chaveia no cuidado planejado, que ela não tem.
