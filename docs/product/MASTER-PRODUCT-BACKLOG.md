...
| F35 | **Avaliação capilar ampliada** | **COMMITTED** | D-102. A lista do dono foi **medida contra `hair_profiles`** (8 inputs, D-62) e a maior parte **já é coletada**: curvatura e espessura, oleosidade (`scalp_tendency`), química, calor, ressecamento/quebra/frizz/embaraço/opacidade (`current_concerns`), frequência de lavagem e objetivo; "mudanças percebidas ao longo do tempo" já são `F17` + `F23`. **O que falta é curto e nomeável: porosidade percebida · rotina e disponibilidade real de tempo** — e, se o dono quiser, o objetivo deixar de ser único. ⚠️ **Uma pergunta que o motor não usa é uma pergunta que só custa o tempo dela** (D-47/D-48): esta capability só entra **junto** com o `F36`, e a regra que ler o input novo é regra capilar ⇒ nasce `candidate` (D-26). |
