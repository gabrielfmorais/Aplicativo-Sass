...
| IMPORTANT | `tests.unapproved_security_definer_functions()` verifica `prosecdef` + allowlist, mas **não** verifica se o `search_path` está fixado. Uma função DEFINER allowlistada com `search_path` livre passaria no guardrail — vetor de search-path hijacking com CI verde. | Reportado aqui, **corrigido na fatia seguinte** (`fix/definer-search-path-guardrail`, 2026-08-28): novo helper `tests.unpinned_security_definer_functions()` + fixture negativo em `004`. Mitigação local mantida: `050` afirma o pin de `submit_checkin` |
