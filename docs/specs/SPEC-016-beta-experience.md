...
|---|---|
| ID | SPEC-016 |
| Status | **IMPLEMENTADA (2026-08-31)** — as cinco fatias entregues e validadas no DEV real. Aprovada como v0.1 (agente, §0.2/§0.3 — D-88, 2026-08-31), a pedido explícito do dono. Nada aqui muda regra de negócio, schema, autorização ou domínio capilar; é apresentação, e apresentação é reversível arquivo a arquivo. |
...
4. ✅ **Entregue (#62), validado no DEV real** — **Conta + Plan Customization/Premium.** A tela passou a **rolar** (não tinha `ScrollView`: tudo abaixo da dobra era inalcançável no celular), premium lê como adição e não como free degradado (G7), e sem preço na tela — preço vem da loja em runtime (D-83).
5. ✅ **Entregue (#64, #65)** — **Passada de consistência** (`CareTypeMark` unificado, `DevSignIn` nos tokens, AC1 verificado por varredura) e **`improve --full`** (D-93): zero BLOCKER, três contradições doc↔implementação corrigidas.

**As cinco fatias estão entregues. A SPEC-016 está IMPLEMENTADA.**
...
| 2026-08-31 | v0.3 — **Fatia 2 marcada DONE só depois da jornada real** (regra de DONE, CLAUDE.md §0.1 / D-90). Executada e observada no projeto DEV `ayecidupmxmirwfzwtea`: DEV sign-in → onboarding em etapas (8 passos, barra de progresso) → preview → **criar cronograma** (12 cuidados persistidos) → Hoje → cartão de foco → **concluir cuidado** → tag `Feito` + check-in no mesmo cartão + ponto do dia preenchido no week strip → **avaliar 4/5** → **reload completo + novo sign-in** → estado persistido (`Você marcou: 4/5`, desfazer ainda aberto, progresso `1 de 1` no plano e `2` desde o início, provando que a contagem vitalícia atravessa a substituição de plano — SPEC-014 FR7/EC6 — contra o banco real). Nenhum BLOCKER novo encontrado nesta passagem. | agente (§0.3) |
| 2026-08-31 | v0.4 — **SPEC IMPLEMENTADA.** Fatia 4 (#62): a Conta passou a rolar — não tinha `ScrollView`, e tudo abaixo da dobra era inalcançável no celular — e premium passou a ler como adição, sem preço na tela (D-83). Fatia 5 (#64, #65): `CareTypeMark` unificado entre Hoje e preview, `DevSignIn` nos tokens, **AC1 verificado por varredura** (zero literal de cor, zero número solto fora de `design/`), e `improve --full` de checkpoint de beta (D-93) com zero BLOCKER. As quatro OQ (fonte própria, ícones, dark mode, motion) seguem abertas com gatilho — nenhuma virou necessidade. | agente (§0.3) |
