...
| [SPEC-010](SPEC-010-subscription-entitlements.md) | Subscription & Entitlements | Subscription | 9 | **Draft v0.2** (2026-08-29, revisada por `spec-review`) — infra Billing→Subscription→Entitlement + paywall provider-agnósticos. Recomenda aprovação em duas partes: infra (PR-A/B/C) já, app (PR-D/E) após as OQ BLOCKING (provider+custo, preço, 1ª capacidade premium — human gates). Requer label `security` |
