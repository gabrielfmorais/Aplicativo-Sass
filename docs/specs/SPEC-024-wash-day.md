...
- **OQ4** ✅ **Fechada pela SPEC-052 (2026-09-06).** O registro avulso entrou: `scheduled_care_id` passou a ser anulável e a RPC `record_ad_hoc_care` cria a execução. ⚠️ **E o Wash Day não mudou uma linha** — `wash_days` sempre pendurou na **execução**, não no plano, e é por isso que a avulsa abre a mesma tela *"Seu registro"*, com a mesma prateleira e as mesmas técnicas.
