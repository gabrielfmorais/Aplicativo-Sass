...
**Auth — estado real (D-84):** OAuth por browser com PKCE (D-56) é o caminho de login do produto (validado na SPEC-001, no aparelho); **no preview web ele não foi verificado ponta a ponta** — ver runbook §5. **O login por email NÃO está funcionando**: o Supabase DEV envia **Magic Link** pelo template padrão, não o código de 6 dígitos que a tela pede, e **custom SMTP não foi configurado**. Não documentar, não testar e não relatar OTP por email como funcional até o template emitir `{{ .Token }}`. Ver `docs/runbooks/WEB-DEV-PREVIEW.md`.
